...
async function createAndCompleteTeam(teamName, memberCount) {
  ...
  assert.ok(granted >= 54);
  assert.ok(Object.values(rawArmorCounts).every((count) => count >= 9));
  ...
  for (const member of members) {
    const lockedDraw = await call("claim-qr", { playerId: member.id, code: "draw-3" });
    assert.ok(lockedDraw.results.length >= 0 && lockedDraw.results.length <= 3);
    assertWarSystemShape(lockedDraw);
  }
...
async function run() {
  ...
  try {
    ...
    for (const route of ["/party", "/forest", "/team/roles", "/team/merge", "/exchange/1", "/draw/1", "/draw/2", "/draw/3", "/mission/judgment", "/mission/endurance", "/mission/speed", "/mission/teamwork", "/mission/intellect", "/mission/power", "/hidden/forest-cache-1", "/boss"]) {
      ...
      assert.match(html, /<title>(?:\[TEST\] )?전신갑주를 입어라<\/title>/);
