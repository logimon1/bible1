...
// [TEST BUILD] 솔로/소수 인원 테스트용 최소 조원 수 (기본 4). MIN_ROSTER=1 로 실행하면 혼자 확정 가능.
const MIN_ROSTER = Math.max(1, Number(process.env.MIN_ROSTER) || 4);
// [TEST BUILD] 테스트 모드: 역할 배정의 "1인 최대 2파트" 등 다인원 제약을 풀어 혼자 6파트 배정 가능.
...
function finalizeTeamRoster(data, player) {
  ...
  const members = allTeamMembersFor(data, player);
  if (members.length < MIN_ROSTER || members.length > 6) {
    throw new Error(`팀원 ${MIN_ROSTER}~6명이 모두 로그인한 뒤 명단을 확정해주세요.`);
  }
...
function markTeamWarScan(data, player) {
  ...
  const allScanned = rosterIds.length >= MIN_ROSTER && rosterIds.every((playerId) => scannedIds.includes(playerId));
...
function setTeamWarRole(data, player, armorCode, selected) {
  ...
  if (selected) {
    ...
    if (!TEST_MODE && myCount >= 2) throw new Error("한 사람은 최대 2개 파트까지 맡을 수 있습니다.");
    ...
    if (!TEST_MODE && remainingParts < membersWithoutRole) {
  ...
  const rolesComplete = Object.keys(assignments).length === ARMOR.length
    && rosterIds.every((playerId) => roleCounts.get(playerId) >= 1 && (TEST_MODE || roleCounts.get(playerId) <= 2));
