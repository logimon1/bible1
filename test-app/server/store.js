...
// [TEST BUILD] 솔로/소수 인원 테스트용 게이트 완화 (api.js와 동일 값 사용).
...
function normalizedTeamSetting(setting, playerIds) {
  ...
  const allScanned = activeRosterIds.size >= MIN_ROSTER && [...activeRosterIds].every((playerId) => warScannedPlayerIds.includes(playerId));
  ...
  const rolesComplete = Object.keys(warRoleAssignments).length === ARMOR.length
    && [...activeRosterIds].every((playerId) => roleCounts.get(playerId) >= 1 && (TEST_MODE || roleCounts.get(playerId) <= 2));
  return {
