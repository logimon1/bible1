...
function checkFiles() {
  [
    ...
    "public/app.js",
    "public/styles.css",
    "public/retreat-theme.css",
    "public/assets/ui/entry-armor-hero.png",
    "api/app.js",
    "server/api.js",
    "server/core.js",
    "server/store.js"
...
function checkVercel() {
  ...
  ["/mission/:code*", "/hidden/:code*", "/exchange/:booth", "/ranking", "/team-ranking", "/monthly", "/forest", "/admin/print/:page"].forEach((route) => {
...
function checkEnvironment() {
  ...
  if (process.env.DELIVERY_BASE_URL) {
    try {
      const url = new URL(process.env.DELIVERY_BASE_URL);
      if (url.protocol === "https:") pass("DELIVERY_BASE_URL is a valid HTTPS URL");
      else warn("DELIVERY_BASE_URL should use HTTPS for printed QR codes");
    } catch {
      fail("DELIVERY_BASE_URL must be a valid URL");
    }
  } else {
    warn("DELIVERY_BASE_URL is not set; QR printing will use the browser origin");
  }
}
