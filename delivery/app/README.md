...
> 이 폴더는 교회 납품·판매용 제품화 앱입니다. 이번 수련회 실사용 앱은 `../../current-retreat/`에서 별도로 관리합니다.

수련회 또는 한 달 챌린지에서 사용할 수 있는 웹앱형 QR RPG MVP입니다. 학생은 휴대폰 기본 카메라로 QR을 찍어 미션에 접속하고, 서버 기준으로 보상/장비/랭킹이 반영됩니다.

Vercel 프로젝트의 Root Directory는 `delivery/app`으로 설정합니다.
...
저장소 루트에서 시작했다면 먼저 `cd delivery/app`을 실행하고, 아래 명령을 이 폴더 안에서 사용합니다.
...
- `DELIVERY_BASE_URL`: 인쇄 QR에 넣을 최종 HTTPS 앱 주소. 미설정 시 현재 브라우저 주소를 사용하며 로컬 주소에서는 인쇄가 잠깁니다.
