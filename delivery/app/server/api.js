  ...
  return withData(async (data) => {
    ...
    if (action === "create-player") {
      ...
      if (submittedCode && !/^\d{4}$/.test(submittedCode)) {
        throw new Error("재접속 코드는 4자리 숫자입니다.");
      }
      let player = findPlayerByIdentity(data, { name, team, accessCode: submittedCode });
      const matches = findPlayersByNameTeam(data, name, team);
      if (!player && submittedCode) {
        throw new Error("이름, 조, 재접속 코드가 일치하는 학생을 찾을 수 없습니다.");
      }
      if (!player && matches.length > 0) {
        throw new Error("이미 같은 이름/조의 캐릭터가 있습니다. 기존 캐릭터는 4자리 재접속 코드를 입력해주세요.");
      }
      if (!player) {
        player = {
          id: id("p"),
...
async function handleApiRequest(req, res) {
  ...
  try {
    ...
    if (action === "qr-svg") {
      ...
      const marginParam = query.get("margin");
      const widthParam = query.get("width");
      const requestedMargin = marginParam === null ? Number.NaN : Number(marginParam);
      const requestedWidth = widthParam === null ? Number.NaN : Number(widthParam);
      const margin = Number.isFinite(requestedMargin) ? Math.max(4, Math.min(16, Math.trunc(requestedMargin))) : 4;
      const width = Number.isFinite(requestedWidth) ? Math.max(220, Math.min(2048, Math.trunc(requestedWidth))) : 512;
      const svg = await QRCode.toString(text, { type: "svg", errorCorrectionLevel: "M", margin, width });
      sendSvg(res, 200, svg);
      return;
    }
