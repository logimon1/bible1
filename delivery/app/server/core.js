...
const { loadProgramConfig, missionUrlPath, publicProgramConfig } = require("./config");

const PROGRAM_CONFIG = loadProgramConfig();
const EQUIPMENT_CONFIG = Object.fromEntries((PROGRAM_CONFIG.equipmentSet || []).map((item) => [item.id, item]));

const BASE_ARMOR = [
  { code: "belt", name: "진리의 허리띠", meaning: "거짓을 분별하는 힘", ability: "말씀 분별력", icon: "/assets/armor/belt.png" },
  { code: "breastplate", name: "의의 흉배", meaning: "정죄와 죄책감에서 보호", ability: "의로움 방어력", icon: "/assets/armor/breastplate.png" },
  { code: "shoes", name: "평안의 복음의 신", meaning: "복음을 전하는 발걸음", ability: "복음 발걸음", icon: "/assets/armor/shoes.png" },
  ...
  { code: "sword", name: "성령의 검", meaning: "하나님의 말씀으로 싸움", ability: "말씀 공격력", icon: "/assets/armor/sword.png" }
];

const ARMOR = BASE_ARMOR.map((armor) => {
  const equipment = EQUIPMENT_CONFIG[armor.code] || {};
  return {
    ...armor,
    name: equipment.name || armor.name,
    verse: equipment.verse || "",
    description: equipment.description || armor.meaning,
    effect: equipment.effect || armor.ability,
    unlockCondition: equipment.unlockCondition || "",
    printText: equipment.printText || ""
  };
});
...
const QR_REWARDS = PROGRAM_CONFIG.qrSet.map((mission, index) => ({
  code: mission.code,
  type: mission.phase === "hidden" ? "hidden" : mission.phase === "boss" ? "boss" : "mission",
  number: index + 1,
  mode: mission.mode || "retreat",
  phase: mission.phase || "",
  weekIndex: Number(mission.weekIndex || 0),
  title: mission.title,
  description: mission.shortDescription || mission.description || "",
  shortDescription: mission.shortDescription || mission.description || "",
  verse: mission.verse || "",
  armor: mission.armor || "",
  reward: mission.reward || {},
  smallGroupQuestion: mission.smallGroupQuestion || "",
  urlPath: missionUrlPath(mission)
}));

const QR_REWARD_MAP = Object.fromEntries(QR_REWARDS.map((reward) => [reward.code, reward]));

const FOREST_TRIALS = [
  { armor: "belt", demon: "성급한 단정", mission: "확인할 수 있는 사실과 아직 확인하지 않은 설명을 구분하고, 사랑의 다음 행동을 정합니다.", benefit: "진리의 힌트를 먼저 볼 수 있습니다." },
...
function publicQrReward(reward) {
  return {
    ...
    verse: reward.verse,
    armor: reward.armor,
    smallGroupQuestion: reward.smallGroupQuestion,
    urlPath: reward.urlPath,
