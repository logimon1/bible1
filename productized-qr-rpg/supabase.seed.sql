...
insert into missions (code, program_id, mode, phase, week_index, title, short_description, verse, armor_code, reward, small_group_question, active)
values ('hidden-forest-cache-1', 'default', 'retreat', 'hidden', 1, '숨겨진 숲 보급품 1', '시련의 숲 주변 히든 QR 보상입니다.', '에베소서 6:13', 'shield', '{"draws":2,"talent":10}'::jsonb, '어려움 속에서도 붙드는 믿음은 무엇인가요?', true)
on conflict (code) do update set
  program_id = excluded.program_id,
  mode = excluded.mode,
  phase = excluded.phase,
