...
async function loadPostgresData(client) {
  const [playersResult, inventoryResult, drawLogsResult, qrClaimsResult, rewardTransactionsResult, eventLogsResult, boothsResult] = await Promise.all([
    ...
    client.query("select id, player_id, source, talent, draw_count, result, created_at from reward_transactions order by created_at"),
  ...
  const rewardTransactions = rewardTransactionsResult.rows;
  ...
  data.rewardTransactions = rewardTransactions.map((row) => ({
    id: row.id,
    playerId: row.player_id,
    source: row.source,
    talent: Number(row.talent || 0),
...
async function savePostgresData(client, data) {
  ...
  const rewardTransactions = (data.rewardTransactions || []).map((transaction) => ({
    id: transaction.id,
    player_id: transaction.playerId,
    source: transaction.source || "",
    talent: Number(transaction.talent || 0),
    draw_count: Number(transaction.drawCount || 0),
    result: { results: transaction.results || [], promotions: transaction.promotions || [] },
  ...
  if (rewardTransactions.length) {
    await client.query(`
      ...
      on conflict (id) do nothing
    `, [JSON.stringify(rewardTransactions)]);
  }
  if (eventLogs.length) {
    await client.query(`
