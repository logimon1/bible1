...
function renderCreate() {
  app.innerHTML = `${topbar()}${programBannerHtml()}<div class="title">시련의 숲<br/>전신갑주 전략전</div>
...
function renderHome() {
  ...
  app.innerHTML = `${topbar()}${programBannerHtml()}
    <div class="panel hero">
      ...
      <div class="profile-meta">${me.team ? `<span class="tag">${escapeHtml(me.team)}</span>` : ""}<span class="tag">달란트 ${formatNumber(me.talent || 0)}</span><span class="tag">갑주 ${me.ownedArmorCount}/6</span>${me.accessCode ? `<span class="tag">입장코드 ${escapeHtml(me.accessCode)}</span>` : ""}</div>
...
function qrImage(url) {
  return `/api/app?action=qr-svg&text=${encodeURIComponent(url)}`;
}

function renderPrintQr() {
  const cards = state.qrRewards.map((mission) => {
    const url = absoluteUrl(mission.urlPath || `/qr/${mission.code}`);
    return `<article class="print-card qr-print-card">
      <div class="print-number">${mission.number || ""}</div>
      <div>
        <h2>${escapeHtml(mission.title)}</h2>
        <p>${escapeHtml(mission.shortDescription || mission.description || "")}</p>
        <p class="print-verse">${escapeHtml(mission.verse || "")}</p>
        <p class="row-sub">${escapeHtml(state.program?.programMode || "")}${mission.weekIndex ? ` · ${mission.weekIndex}주차` : ""}</p>
      </div>
      <img class="print-qr" src="${qrImage(url)}" alt="QR">
  ...
  app.innerHTML = `<div class="print-page">
    ...
    <h1>${escapeHtml(state.program?.eventName || "QR 미션 카드")}</h1>
...
async function adminUpdatePlayer() {
  ...
  try {
    const payload = await api("admin-update-player", { pin: localStorage.getItem(STORAGE_PIN) || "", playerId, name, team, active });
    toast("참가자 수정 완료");
    renderAdmin(payload);
  } catch (error) {
    toast(error.message, "error");
...
async function adminAdjustTalent() {
  ...
  try {
    const payload = await api("admin-adjust-talent", { pin: localStorage.getItem(STORAGE_PIN) || "", playerId, delta });
    toast("달란트 수정 완료");
    renderAdmin(payload);
  } catch (error) {
    toast(error.message, "error");
...
async function adminExportCsv() {
  try {
    const payload = await api("admin-export-csv", { pin: localStorage.getItem(STORAGE_PIN) || "" });
...
async function adminResetData() {
  ...
  try {
    const payload = await api("admin-reset-data", { pin: localStorage.getItem(STORAGE_PIN) || "", confirm: confirmText });
    toast("전체 데이터 초기화 완료");
    renderAdmin(payload);
  } catch (error) {
    toast(error.message, "error");
